...
router.post('/getSingleNFT',async(req,res)=>{
    try{
        ...
        if(contract_address && token_id && chain){
            ...
            // if found in DB then return from DB


            // else fetch from api and add it on object
...
// router.post('/addGamingNft',async(req,res)=>{
//     try{
       

//           const {contract_address,token_id,chain} = req.body;
//           if(contract_address && token_id && chain){
//               const resp = await getSingleNFT(contract_address,token_id,chain); 
//               const nftObject =  {
//                 nftName:resp.token_name,
//                 nftContractAddress: contract_address,
//                 nftTokenId: token_id,
//                 nftRarityNumber: resp?.rarity_rank?resp.rarity_rank:"null",
//                 nftDescription: resp?.token_description?resp.token_description:"null",
//                 nftRarityRank: resp?.rarity_rank?resp.rarity_rank:"null",  
//                 nftPrice: ( resp?.recent_price?.price)?(resp.recent_price.price+" "+resp.recent_price.price_currency):"null",
//                 nftPriceInUsd: resp?.recent_price?.price_usd?resp.recent_price.price_usd:"null",
//                 nftLastSale:resp?.recent_price?.date?resp.recent_price.date:"null",
//                 owners:'-',
//                 held:'-',
//                 nftImage:resp?.cached_images?.original?resp.cached_images.original:"null",
//               };
//               res.json(resp);
//           }
//           else{
    ...
    try{
        ...
        if(  blockSpanKey && chain ){
          ...
          if(singleExchangeResp){
              ...
              let carouselCollectionObj = await new GamingCollections({
                  contractAddress: singleExchangeResp.contracts[0]?.contract_address,
                  blockSpanKey:blockSpanKey,
                  name: singleExchangeResp.name,
                  chain:chain
